...
import matplotlib.pyplot as plotLSTM
...
def print_tvars():
  ...
  for variable in tvars:
    print(variable.name)
  return
def execute_LSTM(_timings_array):
    """Hyperparameters"""
    ...
    UCR = True
    if UCR:
      dataset = "VideoSummaryTrain"
      datadir = 'LSTM/videosummary/'+ dataset + '/' + dataset
    ...
    if True:  #Set true if you want to visualize the actual time-series
        #f, axarr = plotLSTM.subplots(plot_row, num_classes)
        for c in np.unique(y_train):    #Loops over classes, plot as columns
            ...
            for n in range(plot_row):  #Loops over rows
                ...
                #if not n == plot_row-1:
                    #plotLSTM.setp([axarr[n,c].get_xticklabels()], visible=False)
                #if not c == 0:
                    #plotLSTM.setp([axarr[n,c].get_yticklabels()], visible=False)
    ...
    print("--------------------LSTM Video Series Prediction--------------------------------")
    print_tvars()
    ...
    if(acc_test_before<acc_test):
        print("--------------------LSTM Series Used for videoClassification is GOOD")

    #plotLSTM.figure()
    #plotLSTM.plot(perf_collect[0],label='Valid accuracy')
    #plotLSTM.plot(perf_collect[1],label = 'Train accuracy')
    #plotLSTM.axis([0, step, 0, np.max(perf_collect)])
    #plotLSTM.legend()
    #print("--------------------LSTM Video Series Prediction--------------------------------")
    #plotLSTM.show()
